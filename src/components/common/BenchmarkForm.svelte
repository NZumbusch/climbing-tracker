...
<!-- UI Structure -->
<div class="space-y-4 p-5 bg-zinc-900/80 rounded-3xl border border-zinc-800 animate-in zoom-in-95 shadow-2xl">
  <div class="flex items-center justify-between px-1">
    <h3 class="text-sm font-bold text-white tracking-tight">Benchmark Test</h3>
    <button onclick={onCancel} class="text-zinc-500 hover:text-white transition-colors">
  ...
  <div class="space-y-3">
    <div class="space-y-1">
      <label for="benchmark-type" class="text-[8px] font-black text-zinc-500 uppercase tracking-widest ml-1">Type</label>
      <select 
        id="benchmark-type"
        bind:value={benchmark.typeId} 
        onchange={handleTypeChange}
        class="w-full bg-zinc-800 text-white p-3 rounded-xl border border-zinc-700 outline-none text-sm appearance-none"
      >
    ...
    <div class="flex gap-3">
      <div class="flex-1 space-y-1">
        <label for="benchmark-value" class="text-[8px] font-black text-zinc-500 uppercase tracking-widest ml-1">Result</label>
        <input 
          id="benchmark-value"
          type="number" 
          bind:value={benchmark.value} 
          class="w-full bg-zinc-800 text-white p-3 rounded-xl border border-zinc-700 outline-none text-sm"
        />
      </div>
      <div class="w-24 space-y-1">
        <label for="benchmark-unit" class="text-[8px] font-black text-zinc-500 uppercase tracking-widest ml-1">Unit</label>
        <input 
          id="benchmark-unit"
          bind:value={benchmark.unit} 
          readonly
          class="w-full bg-zinc-700/50 text-zinc-400 p-3 rounded-xl border border-zinc-700 outline-none text-sm"
        />
      </div>
    </div>

    <div class="space-y-1">
      <label for="benchmark-notes" class="text-[8px] font-black text-zinc-500 uppercase tracking-widest ml-1">Notes</label>
      <textarea 
        id="benchmark-notes"
        bind:value={benchmark.notes} 
        class="w-full bg-zinc-800 text-white p-3 rounded-xl border border-zinc-700 outline-none text-sm h-20"
      ></textarea>
    </div>
  </div>
  ...
  <button 
    ...
    class="w-full py-3.5 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg active:scale-95"
  >
    Save Benchmark
  </button>
</div>
